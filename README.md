...
- **MIX** — the key beside the keypad on either watching screen (on the handset, on a phone). Teletext was never a screen of its own: it was drawn *into* the picture already on the tube, and every cell coded black as its background was a hole the broadcast showed through. That's what MIX does here. There is no broadcast behind the archive, so the live picture is **your webcam** — asked for when you press MIX, handed straight back when you press it again or switch the set off, and never sent anywhere: the stream goes to a `<video>` element and nothing reads it, samples it or shares it. Refuse the camera and the tube says `NO SIGNAL`, then goes back to black. **The published site is built without this key**, because asking a visitor for their camera is a decision worth making on purpose rather than by default; build with `VITE_MIX=on` to put it back on the panel (see `.env.example` and `src/features.ts`).
